namespace :katello do
  ...
  task :create_pulp_repos => ["environment", "dynflow:client"] do
    ...
    if ENV['VERBOSE']
      verbose = ["1", "true"].include? ENV['VERBOSE'].downcase
    else
      verbose = false
    end
    ...
    done_count = 0
    puts ""
    # Check if pulp2 or pulp3 is preferred, using the first repo on the list as an example
    first_repo = repolist.first
    def prefer_pulp2?(capsule, repo)
      capsule.pulp2_preferred_for_type? repo.repository_type.id
    end
    if internal_capsule.pulp3_enabled? and !prefer_pulp2?(internal_capsule,first_repo)
      repolist.each do
        |onerepo|
        task = ForemanTasks.async_task(Actions::Pulp3::Repository::Create, onerepo, internal_capsule)  # pulp3 accepts a "force" param
        done_count +=1
        printf "\rRepos scheduled so far: #{done_count}/#{repolist.count}" if done_count % 10 == 0
      end
      printf "\rRepos scheduled so far: #{done_count}/#{repolist.count}\n"
    else
      repolist.each do
        |onerepo|
